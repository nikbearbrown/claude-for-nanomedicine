...
![Imaging modalities ranked by spatial resolution across seven orders of magnitude, with the 200 nm diffraction floor dividing optical from sub-optical instruments](images/01-the-resolution-ladder-fig-01.png)
*Figure 1.1 — The Resolution Ladder*
...
![Visible light bottoms out at the 200 nm Abbe floor while a 100 kV electron's far shorter wavelength permits much finer resolution](images/01-the-resolution-ladder-fig-02.png)
*Figure 1.2 — Why Wavelength Sets the Resolution Ceiling*
...
![As field of view grows from one cell to the whole body, resolution coarsens by seven orders of magnitude, leaving the high-res large-field corner empty](images/01-the-resolution-ladder-fig-03.png)
*Figure 1.3 — Resolution vs. Field of View: The Inverse Coupling*
...
### Figure 1.1 — The Resolution Ladder
Build a horizontal dot/lollipop chart ranking nine imaging modalities by spatial resolution, expressed as log10 of resolution in nanometers, spanning roughly -1 (electron microscopy, ~0.1 nm) to 6.7 (PET, ~6 mm) along a single horizontal log axis. Data shape: one categorical row per modality (Electron microscopy/TEM, SEM/AFM, Super-resolution, Confocal/light microscopy, Histology, Ultrasound, CT, MRI, PET), each with one numeric value. Marks: a labeled dot at each value connected to the axis by a thin stem. Sort rows by value ascending (finest at top). No zero baseline — this is a position-on-log-axis chart, not a magnitude bar. Draw a vertical reference line at value 2.3 labeled "200 nm diffraction floor" dividing optical from sub-optical instruments. Color semantics (Okabe-Ito): blue for the dominant electron-microscopy anchor, green for positive/signal modalities (super-resolution, PET), sky-blue for the confocal anchor at the floor, muted gray for secondary clinical modalities. Annotate the two extreme rows with their real resolution in physical units. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables.

### Figure 1.2 — Why Wavelength Sets the Resolution Ceiling
Build a two-panel comparison contrasting a visible-light probe (left) against an accelerated-electron probe (right). Data shape: three paired rows, each a left-label/right-label pair on a shared theme — (1) wavelength: ~500 nm vs ~0.004 nm; (2) resolution outcome: ~200 nm Abbe floor vs theoretical ceiling far finer; (3) practical limit: diffraction-limited vs ~0.1-0.2 nm aberration floor. Layout: two vertical columns titled "Visible light" and "Accelerated electron," with the three rows aligned horizontally so each comparison reads across. Marks: text-bearing cells or short horizontal bars sized to the log wavelength where applicable. Use a sky-blue anchor for the wavelength row, gray for the resolution-outcome row, vermillion for the negative/limit row (diffraction and aberration floors). No continuous shared axis required; emphasize the side-by-side contrast. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables.

### Figure 1.3 — Resolution vs. Field of View: The Inverse Coupling
Build a node-edge conceptmap (systems diagram) showing the inverse coupling between resolution and field of view. Data shape: four nodes positioned in a 2D resolution-versus-field space — EM (fine resolution, tiny field, upper-left), light microscopy/histology (middle), CT/MRI/PET (coarse resolution, whole-body field, lower-right), and an explicitly empty corner node (high-res + large field, upper-right) labeled impossible. Edges: directed arrows from EM to light microscopy ("field grows") and from light microscopy to clinical ("resolution coarsens") tracing the diagonal; a blocked/struck edge from clinical to the empty corner labeled "no instrument lives here." Channels: x-position encodes field of view, y-position encodes resolution. Color (Okabe-Ito): blue for the dominant EM node, sky-blue anchor for the middle, gray for clinical, vermillion for the impossible empty corner and its blocked edge. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables.
