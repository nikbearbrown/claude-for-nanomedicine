...
![Diagnosis runs as a chain from clinical suspicion through imaging, biopsy, and pathology to diagnosis; only pathology of cells can label a target cancer](images/02-anatomic-imaging-ct-ultrasound-and-clinical-context-fig-03.png)
*Figure 2.3 — The Diagnostic Chain: Imaging Is a Step, Not an Endpoint*
...
![A CT scanner acquires X-ray attenuation from many angles, then solves the inverse problem to reconstruct a density map where bone is bright and air is black](images/02-anatomic-imaging-ct-ultrasound-and-clinical-context-fig-01.png)
*Figure 2.1 — CT Signal Formation and Reconstruction*
...
![Higher-frequency ultrasound resolves fine detail but dies shallow, while lower frequency reaches deep organs at coarser resolution](images/02-anatomic-imaging-ct-ultrasound-and-clinical-context-fig-02.png)
*Figure 2.2 — The Ultrasound Frequency-Depth Tradeoff*
...
### Figure 2.1 — CT Signal Formation and Reconstruction
Build a left-to-right flowchart showing CT signal formation across three stages. Data shape: three ordered stages connected by two directed arrows labeled "acquire" and "reconstruct" — (1) angular X-ray acquisition with source/detector orbiting a slice; (2) stacked one-dimensional projection profiles; (3) reconstructed two-dimensional density image with bone bright, air black, soft tissue gray. Layout: three boxes flowing left to right, each numbered, with arrows between. Channels: position-in-sequence is the only spatial encoding. Add a caption note: reconstruction solves the inverse problem from many angular projections. Color (Okabe-Ito): sky-blue anchor for the acquisition stage, blue for the dominant projection stage, green for the positive reconstructed-output stage. Include a small schematic glyph in each box (orbiting ring, stacked lines, gray-scale density map). Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables.

### Figure 2.2 — The Ultrasound Frequency-Depth Tradeoff
Build a two-panel comparison contrasting high-frequency (left) against low-frequency (right) ultrasound. Data shape: three paired rows aligned across the two columns — (1) resolution: fine vs coarse; (2) penetration: shallow vs deep; (3) outcome: signal dies at depth limit vs reaches deep organs. Layout: two titled columns ("High frequency," "Low frequency") with three horizontal comparison rows. Optionally render each side as a stylized depth-vs-resolution bar so the inverse coupling is visible: high frequency = tall resolution, short depth; low frequency = short resolution, tall depth. Color (Okabe-Ito): green for the positive resolution row, blue for the dominant penetration row, vermillion for the negative depth-limit row. Emphasize that the tradeoff is fixed by tissue attenuation physics, not technology. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables.

### Figure 2.3 — The Diagnostic Chain: Imaging Is a Step, Not an Endpoint
Build a left-to-right flowchart of five ordered stages connected by directed arrows: clinical suspicion → imaging (locate and measure) → biopsy → pathology → diagnosis. Data shape: five sequential nodes, each numbered, single linear path with no branches. Channels: position-in-sequence encodes the diagnostic order. Annotate that imaging only locates and measures the target; only pathology of cells can label it cancer. Color (Okabe-Ito): gray for clinical suspicion and biopsy as secondary steps, sky-blue anchor for the imaging step, green for pathology and diagnosis as the positive endpoints. Add a caption note: imaging finds and measures; pathology delivers the cellular answer. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables.
