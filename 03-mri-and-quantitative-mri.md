...
![Protons align in the static field, an RF pulse tips them, then relaxation forks into T1 realignment (fat bright) and T2 dephasing (fluid bright)](images/03-mri-and-quantitative-mri-fig-02.png)
*Figure 3.2 — T1 vs T2: Two Relaxation Processes, Two Images*
...
![After effective therapy the apparent diffusion coefficient rises in weeks 2-4 as dying cells loosen, weeks before tumor size falls on structural imaging](images/03-mri-and-quantitative-mri-fig-01.png)
*Figure 3.1 — ADC Rises Before the Tumor Shrinks*
...
![DCE-MRI curve shape differs sharply between normal tissue, a high-grade tumor, and a responding tumor, encoding vascular permeability no structural image shows](images/03-mri-and-quantitative-mri-fig-03.png)
*Figure 3.3 — DCE-MRI: Curve Shape Encodes Vascular Physiology*
...
### Figure 3.1 — ADC Rises Before the Tumor Shrinks
Build a horizontal timeline showing four sequential markers tracking treatment response over time from treatment start. Data shape: four ordered events along a single time axis — (1) baseline: ADC low, size flat; (2) weeks 2-4: ADC rises as dying cells loosen and water moves freely; (3) lead-time gap: signal ahead of size; (4) later: tumor size falls as a lagging structural proxy. Layout: a left-to-right horizontal timeline with labeled marker nodes and subcaptions. Optionally overlay two offset traces — a rising ADC curve that turns up early and a size curve that falls later — to make the lead-time gap visible. Channels: x-position encodes time; the vertical offset between the two traces is the key annotation. Color (Okabe-Ito): sky-blue anchor for baseline, green for the rising-ADC positive signal, gray for the lagging size trace; annotate the lead-time gap explicitly. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables.

### Figure 3.2 — T1 vs T2: Two Relaxation Processes, Two Images
Build a left-to-right flowchart that forks. Data shape: two shared upstream stages then a two-way fork — (1) protons aligned in static field; (2) RF pulse tips protons perpendicular; then the path forks into (3) T1 relaxation: realign, rising recovery, fat bright; and (4) T2 relaxation: dephase, falling decay, fluid bright. Layout: two stacked boxes flowing into a fork node that branches into two parallel relaxation branches drawn side by side, each with a small recovery/decay curve glyph (T1 rising-to-plateau, T2 falling-decay). Channels: position-in-sequence plus the fork topology. Add a caption note: after the RF tip the process forks into two parallel relaxation branches. Color (Okabe-Ito): sky-blue anchor for the two shared upstream stages, blue for the dominant T1 branch, orange/transitional for the T2 branch. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables.

### Figure 3.3 — DCE-MRI: Curve Shape Encodes Vascular Physiology
Build a comparison pairing three curve types against their vascular meaning, ideally rendered as three overlaid signal-intensity-versus-time traces sharing one axis. Data shape: three curves with paired interpretations — (1) normal tissue: slow gradual rise to plateau = intact low-permeability vasculature; (2) high-grade tumor: rapid peak then rapid washout = leaky aggressive vasculature; (3) responding tumor: reduced peak, slow washout = treatment working. Layout: a single line chart with time-from-contrast-injection on x and signal intensity on y, three distinguishable traces, plus a small legend mapping each curve to its vascular meaning. Zero baseline yes — signal intensity rises from a true zero. Channels: curve shape (slope, peak height, washout rate) is the encoded variable. Color (Okabe-Ito): sky-blue anchor for normal tissue, vermillion for the negative aggressive high-grade curve, green for the positive responding curve. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables.
