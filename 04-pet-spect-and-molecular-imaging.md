...
![A positron emitted by the tracer annihilates with an electron into two back-to-back 511 keV photons that a detector ring records as coincident lines reconstructing an activity map](images/04-pet-spect-and-molecular-imaging-fig-01.png)
*Figure 4.1 — The PET Signal Chain: Decay to Coincidence Detection*
...
![FDG enters through glucose transporters and is phosphorylated, but the modified molecule cannot continue through glycolysis, so it is trapped and accumulates in proportion to glucose uptake](images/04-pet-spect-and-molecular-imaging-fig-02.png)
*Figure 4.2 — FDG: Why the Glucose Analog Gets Trapped*
...
![FDG reads generic glucose hunger shared by tumor and inflammation, producing a noisy field, while a target-specific PSMA tracer lights one sharp deposit against a dark background](images/04-pet-spect-and-molecular-imaging-fig-03.png)
*Figure 4.3 — Generic vs Target-Specific Tracer: FDG vs PSMA*
...
### Figure 4.1 — The PET Signal Chain: Decay to Coincidence Detection
Build a left-to-right flowchart of five ordered stages connected by directed arrows tracing the PET signal physics. Data shape: (1) nucleus emits positron; (2) positron travels a short range, then annihilates with an electron; (3) two 511 keV photons fly back-to-back at 180 degrees; (4) detector ring records the coincident pair, defining a line; (5) many lines reconstruct a hot activity region. Layout: five numbered boxes flowing left to right, with a small schematic glyph in each (positron emission, annihilation burst, two opposed photon arrows, detector ring with a chord line, reconstructed hot region). Channels: position-in-sequence. Annotate that the positron's short travel before annihilation sets the resolution blur (~4-6 mm). Color (Okabe-Ito): green for the positive emission and photon stages, vermillion for the negative annihilation/positron-range step, sky-blue anchor for the detector-ring stage, blue for the dominant reconstruction stage. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables.

### Figure 4.2 — FDG: Why the Glucose Analog Gets Trapped
Build a left-to-right flowchart with a blockage point showing FDG metabolic trapping. Data shape: four ordered stages with relations enter → blocked → pool — (1) FDG enters via glucose transporter; (2) phosphorylation modifies the molecule; (3) glycolysis exit blocked; (4) trapped molecules accumulate. Layout: four boxes left to right, with the third transition drawn as a struck/blocked arrow (a barrier glyph) rather than a normal arrow to mark where glycolysis halts. Channels: position-in-sequence plus the blocked-edge encoding. Annotate that accumulation is proportional to glucose uptake. Color (Okabe-Ito): green for the positive entry stage, sky-blue anchor for phosphorylation, vermillion for the negative blocked-exit stage, blue for the dominant accumulation stage. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables.

### Figure 4.3 — Generic vs Target-Specific Tracer: FDG vs PSMA
Build a two-panel comparison imaging the same hidden small lesion under two tracer regimes. Data shape: three paired rows across two columns ("Generic metabolic (FDG)," "Target-specific (PSMA)") — (1) binding: binds tumor, inflammation, and background alike vs binds only the cancer's distinctive molecule; (2) field appearance: noisy uniformly lit low contrast vs dark clean field with one sharp deposit; (3) outcome: true lesion lost in the field vs true lesion isolated and bright. Layout: two side-by-side panels, each optionally rendering a stylized field — FDG panel as a noisy uniformly bright background, PSMA panel as a dark field with one bright dot — plus the three aligned comparison rows below. Channels: contrast and signal isolation are the encoded variables. Color (Okabe-Ito): gray for the secondary binding row, green for the positive clean-field/isolated-lesion row, vermillion for the negative lost-lesion row. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables.
