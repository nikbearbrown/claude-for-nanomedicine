...
![Anatomic CT and molecular PSMA-PET are two readings of the same lesion map that can disagree: a lesion bright on CT but dark on PET is cancer the drug cannot reach](images/05-theranostic-imaging-fig-03.png)
*Figure 5.3 — The Discordant Lesion: CT Shows Cancer, PET Shows Reach*
...
![A single PSMA ligand becomes a diagnostic with a positron-emitting isotope or a therapy with a beta or alpha emitter, driving an image, select, treat, re-image loop](images/05-theranostic-imaging-fig-01.png)
*Figure 5.1 — The Theranostic Loop: One Ligand, Image-Then-Treat*
...
![A beta emitter irradiates a few millimeters around each cell creating crossfire, while an alpha emitter deposits energy within 50-100 micrometers with almost no crossfire](images/05-theranostic-imaging-fig-02.png)
*Figure 5.2 — β vs α Emitters: Range and Crossfire*
...
### Figure 5.1 — The Theranostic Loop: One Ligand, Image-Then-Treat
Build a closed-loop cycle diagram showing the four-step theranostic workflow around a shared PSMA ligand. Data shape: four stages arranged clockwise in a circle — (1) image: identify target-expressing lesions; (2) select: confirm eligibility, flag discordant lesions; (3) treat: administer the therapeutic isotope; (4) re-image: confirm dose, assess response — with directed arrows closing the loop back to step 1. Optionally place a central node representing the shared ligand with two branches feeding the cycle: a positron-emitting diagnostic isotope (gallium-68/fluorine-18) and a beta/alpha therapeutic isotope (lutetium-177/actinium-225). Channels: angular position around the loop encodes workflow order. Color (Okabe-Ito): blue for the dominant image step, sky-blue anchor for select, green for the positive treat step, orange/transitional for re-image. Annotate each step with its physical signal and biological question. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables.

### Figure 5.2 — β vs α Emitters: Range and Crossfire
Build a two-panel comparison contrasting a beta emitter (left) against an alpha emitter (right) across a mixed target-positive/target-negative cell neighborhood. Data shape: three paired rows across two columns ("β emitter," "α emitter") — (1) range: reaches a few millimeters, multi-cell vs reaches 50-100 micrometers, one-cell radius; (2) crossfire: wide crossfire halo with off-target spillover vs tight halo, neighbors spared; (3) consequence: kills target-negative neighbors vs each cell must be directly hit. Layout: two side-by-side panels, each drawing a stylized cell grid with a central targeted cell and a shaded irradiation radius — wide diffuse halo on the beta side, tight halo on the alpha side. Channels: halo radius encodes range; cell shading encodes which neighbors are irradiated. Color (Okabe-Ito): blue for the dominant range row, gray for the secondary crossfire-halo row, green for the positive consequence row. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables.

### Figure 5.3 — The Discordant Lesion: CT Shows Cancer, PET Shows Reach
Build a paired-panel schematic comparison showing the same lesion distribution read two ways. Data shape: three paired rows across two columns ("Anatomic CT reading," "Molecular PSMA-PET reading") — (1) all lesions present and uniform vs most lesions bright (target-positive); (2) lesion clearly detected as a mass vs same lesion dark, target-negative; (3) structural presence confirmed vs drug reaches the bright lesions. Layout: two side-by-side body/lesion-map panels showing the identical set of lesion positions; on the CT panel all lesions appear as uniform masses, on the PET panel most are bright but one is conspicuously dark. Channels: per-lesion brightness is the encoded variable; the single dark lesion in an otherwise lit field is the focal annotation. Color (Okabe-Ito): sky-blue anchor for the shared lesion map, green for the positive drug-reaches/bright lesions, vermillion for the negative discordant dark lesion. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables.
