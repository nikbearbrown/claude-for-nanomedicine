...
![A fluorophore absorbs a photon, loses a little energy as heat, then emits a longer-wavelength photon; this Stokes shift lets a filter separate emission from excitation](images/06-fluorescence-microscopy-fig-01.png)
*Figure 6.1 — The Fluorescence Cycle and the Stokes Shift*
...
![Widefield, confocal, two-photon, and super-resolution sit at different points in resolution-versus-depth space, with no single method occupying the finest-and-deepest corner](images/06-fluorescence-microscopy-fig-02.png)
*Figure 6.2 — The Microscopy Scale Ladder: Resolution vs Depth*
...
![In indirect immunofluorescence the detected photon is a proxy for the fluorophore, then the secondary antibody, the primary, and finally the target protein; any junction can decouple from the truth](images/06-fluorescence-microscopy-fig-03.png)
*Figure 6.3 — The Antibody Proxy Chain: Photon to Protein*
...
### Figure 6.1 — The Fluorescence Cycle and the Stokes Shift
Build a closed-loop cycle diagram pairing the four-step fluorescence energy mechanism with the resulting spectral shift. Data shape: four stages around a cycle — (1) absorption: photon excites electron to a higher state; (2) vibrational relaxation: small energy lost as heat; (3) emission: longer-wavelength photon released; (4) Stokes-shift gap lets a filter separate emission from excitation. Layout: a Jablonski-style energy ladder (ground state, excited state, downward vibrational-relaxation step, emission arrow) on one side, and a paired excitation/emission spectral plot on the other showing two offset peaks with the Stokes shift in nm between them and a filter bandpass drawn over the emission peak. Channels: energy level (vertical) for the mechanism; wavelength (horizontal) for the spectra. Color (Okabe-Ito): blue for the dominant absorption step, gray for the secondary heat-loss step, green for the positive emission step, sky-blue anchor for the Stokes-shift/filter gap. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables.

### Figure 6.2 — The Microscopy Scale Ladder: Resolution vs Depth
Build a node-edge conceptmap positioning four methods in a 2D resolution-versus-depth space. Data shape: five nodes — widefield (coarse ~250 nm, shallow), confocal (coarse-to-moderate, modest depth), two-photon (moderate resolution, deep tissue), super-resolution (finest ~10 nm, shallow-to-modest), and an explicitly empty corner (finest + deepest, no method). Edges: directed arrows widefield→confocal ("cleaner sectioning"), confocal→two-photon ("deeper reach"), confocal→super-resolution ("finer resolution"), and a blocked/struck edge two-photon→empty-corner ("tradeoff"). Channels: x-position encodes sample depth, y-position encodes resolution (finest at top). Color (Okabe-Ito): gray for widefield, sky-blue anchor for confocal, blue for the dominant two-photon node, green for the positive super-resolution node, vermillion for the impossible empty corner and its blocked edge. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables.

### Figure 6.3 — The Antibody Proxy Chain: Photon to Protein
Build a layered schematic chain (read right-to-left, detection back to target) showing the proxy stack in indirect immunofluorescence. Data shape: five stacked layers, each labeled with a note — (1) emitted photon, the detected readout end; (2) fluorophore, which emitted the photon; (3) secondary antibody, which carries the fluorophore; (4) primary antibody, which binds the target; (5) target protein, the truth the chain reports. Layout: five horizontal layers stacked or chained, with connecting links between adjacent layers and a small "can decouple here" marker at each junction to show any binding event may break. Channels: layer order encodes distance from the detected signal back to the biological truth. Color (Okabe-Ito): gray for the photon and fluorophore readout layers, blue for the dominant secondary-antibody layer, sky-blue anchor for the primary antibody, green for the positive target-protein layer. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables.
