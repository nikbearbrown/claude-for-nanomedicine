...
![Flowchart from sheath-flow focusing through laser interrogation, detector split, and a sequential gating tree to a final selected population](images/07-flow-cytometry-and-facs-fig-01.png)
*Figure 7.1 — Flow cytometer fluidics-to-gating pipeline*
...
![Before/after two-panel schematic showing an emission tail spilling into a neighbor channel and then the spillover fraction subtracted to recover true negatives](images/07-flow-cytometry-and-facs-fig-03.png)
*Figure 7.3 — Spectral overlap and compensation*
...
![Two-column comparison of analytical flow/FACS versus mass cytometry across parameters, throughput, and cell recovery](images/07-flow-cytometry-and-facs-fig-02.png)
*Figure 7.2 — Flow vs FACS vs CyTOF capability comparison*
...
### Figure 7.1 — Flow cytometer fluidics-to-gating pipeline
Build a left-to-right flowchart of six sequential stages: sheath-flow focusing → laser interrogation → detector split (FSC/SSC) → scatter plot → sequential gating tree → selected population. Render each stage as a labeled node connected by arrows; at the "sequential gating tree" node, branch downward into a small sub-tree showing each gate passing a retained subset forward and excluding the rest. Number stages 1–6. Use Okabe-Ito semantics: sky-blue for the anchor (sheath focusing), blue for the dominant step (laser interrogation), green for the positive endpoint (selected population), neutral gray for intermediate stages. Add a footnote annotation: "Each gate passes a retained subset forward and excludes the rest." Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables. Structural, not aesthetic.

### Figure 7.2 — Flow vs FACS vs CyTOF capability comparison
Build a two-panel side-by-side comparison matrix. Left column header "Analytical flow / FACS", right column header "Mass cytometry (CyTOF)". Render four aligned rows, each a paired left/right cell: (1) ~15-30 fluorescence parameters vs 40+ metal-isotope parameters; (2) high throughput vs lower throughput; (3) cells survive vs cells ionized/not recoverable; (4) spectral overlap needs compensation vs minimal channel crosstalk. Use Okabe-Ito semantics: green for the positive row (minimal crosstalk side), vermillion for the negative row (cells not recoverable), neutral for the rest. Align rows on a shared horizontal baseline so the tradeoff reads across. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables. Structural, not aesthetic.

### Figure 7.3 — Spectral overlap and compensation
Build a before/after two-panel comparison. Left panel "Uncompensated", right panel "Compensated". Show overlapping fluorophore emission curves where one curve's tail spills into a neighbor detector band; in the right panel show the spillover fraction subtracted so the true-negative population resolves. Render three aligned rows of paired statements: (1) emission tail spills into neighbor channel vs spillover fraction subtracted; (2) negative cells read false-positive vs negatives resolved correctly; (3) cluster mis-placed in wrong quadrant vs cluster pulled to correct quadrant. Use Okabe-Ito semantics: vermillion for the artifact/false-positive (left), green for the corrected/true negative (right). Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables. Structural, not aesthetic.
