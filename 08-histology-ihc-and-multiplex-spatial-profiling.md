...
![Two-row sequential workflow from formalin fixation and paraffin embedding through deparaffinization, antigen retrieval, primary/secondary antibody, and DAB precipitate signal](images/08-histology-ihc-and-multiplex-spatial-profiling-fig-01.png)
*Figure 8.1 — FFPE preparation and IHC staining workflow*
...
![Four-tier ordered ladder of HER2 IHC scores 0, 1+, 2+, 3+ with 2+ marked equivocal and 3+ highlighted as positive](images/08-histology-ihc-and-multiplex-spatial-profiling-fig-02.png)
*Figure 8.2 — HER2 IHC scoring ladder (0 / 1+ / 2+ / 3+)*
...
![Bar chart of simultaneous markers per section rising from single-marker IHC through multiplex immunofluorescence to CODEX and IMC/MIBI at constant single-cell resolution](images/08-histology-ihc-and-multiplex-spatial-profiling-fig-03.png)
*Figure 8.3 — Spatial profiling: markers per section*
...
### Figure 8.1 — FFPE preparation and IHC staining workflow
Build a two-row left-to-right sequential flowchart of eight numbered stages: (1) fix tissue (cross-link) → (2) embed in wax → (3) section and mount → (4) deparaffinize → (5) antigen retrieval (break cross-links) → (6) block → (7) primary + secondary-enzyme → (8) DAB precipitate signal. Render each as a labeled node joined by arrows. Use Okabe-Ito semantics: sky-blue anchor for sectioning, vermillion for antigen retrieval (the step that reverses fixation), blue for the dominant primary/secondary binding step, green for the DAB positive signal endpoint, neutral gray for the rest. Add a footnote: "Fixation cross-links preserve morphology but mask epitopes; retrieval reverses this." Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables. Structural, not aesthetic.

### Figure 8.2 — HER2 IHC scoring ladder (0 / 1+ / 2+ / 3+)
Build a vertical bar chart of four ordered ordinal scores on the x-axis (0, 1+, 2+, 3+) with bar height encoding membrane-staining intensity (values 0, 1, 2, 3). Zero baseline yes; sort by ordinal score ascending left to right. Mark the 2+ bar as the equivocal transitional bin and highlight the 3+ bar as the positive call. Use Okabe-Ito semantics: neutral gray for 0 and 1+, a transitional tone (orange/yellow) for the 2+ equivocal bin, green for the 3+ positive bar. Annotate the 2+ bar with a callout: "equivocal — reflex to ISH". Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables. Structural, not aesthetic.

### Figure 8.3 — Spatial profiling: markers per section
Build a horizontal bar chart of simultaneous markers per section across four methods: single-marker IHC (1), multiplex immunofluorescence (7), CODEX (45), IMC/MIBI (40). Zero baseline yes; sort ascending by marker count. Channel marker count to bar length; annotate that single-cell resolution is constant across all rows. Use Okabe-Ito semantics: neutral/secondary gray for single-marker IHC, blue for the dominant high-depth CODEX bar, green highlight for IMC/MIBI. Add a reference annotation noting constant single-cell resolution despite rising marker depth. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables. Structural, not aesthetic.
