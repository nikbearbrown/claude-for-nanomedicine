...
![Horizontal log-scale resolution ladder placing SEM, AFM, cryo-EM, and TEM by achievable resolution in nanometers](images/09-electron-microscopy-sem-tem-and-afm-fig-01.png)
*Figure 9.1 — SEM / TEM / cryo-EM / AFM resolution comparison*
...
![Layered schematic of the SEM beam-specimen interaction teardrop showing secondary-electron, backscattered-electron, and X-ray escape depths and how lower kV shrinks the cloud](images/09-electron-microscopy-sem-tem-and-afm-fig-02.png)
*Figure 9.2 — SEM beam-specimen interaction volume*
...
![Six-step horizontal flow from intact cell through fixation, osmium post-fixation, dehydration, resin embedding and ultramicrotomy, to heavy-metal staining](images/09-electron-microscopy-sem-tem-and-afm-fig-03.png)
*Figure 9.3 — Conventional TEM sample-prep pipeline*
...
![Workflow and before/after contrast showing vitrified cryo-EM particles staying dispersed versus dried-TEM particles clumping into fused aggregates](images/09-electron-microscopy-sem-tem-and-afm-fig-04.png)
*Figure 9.4 — Cryo-EM vitrification vs dried-TEM artifact*
...
### Figure 9.1 — SEM / TEM / cryo-EM / AFM resolution comparison
Build a horizontal dot/lollipop plot positioning four techniques on a log10 resolution axis (nm): SEM (~3 nm, log 0.5), AFM (~1 nm, log 0), cryo-EM (~0.3 nm, log -0.5), TEM (~0.15 nm, log -0.8). Channel resolution to horizontal position; finer resolution toward the left/lower end of the log axis. Sort by resolution so the ladder reads continuously. Use Okabe-Ito semantics: sky-blue anchor for AFM, neutral/secondary for SEM, green for cryo-EM, blue dominant highlight for TEM (finest). Add an axis annotation clarifying these are different signals at different scales, not competing answers. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables. Structural, not aesthetic.

### Figure 9.2 — SEM beam-specimen interaction volume
Build a layered annotated schematic of a teardrop interaction volume beneath a nominally 1 nm beam. Stack escape-depth layers from shallow to deep with callout notes: secondary electrons (top few nm, topographic signal), backscattered electrons (mid depth, compositional), characteristic X-rays (deepest zone, confound), and a separate anchor annotation for "lower kV shrinks the teardrop, shallows all escape depths". Use Okabe-Ito semantics: green for secondary electrons (useful topographic signal), neutral/secondary for backscattered, transitional tone for X-rays (confound), sky-blue anchor for the lower-kV note. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables. Structural, not aesthetic.

### Figure 9.3 — Conventional TEM sample-prep pipeline
Build a left-to-right sequential flowchart of six numbered stages: (1) intact cell → (2) fixation (cross-link) → (3) osmium post-fixation → (4) dehydration (contraction) → (5) resin embed + ultramicrotomy → (6) heavy-metal stain. Render labeled nodes joined by arrows. Use Okabe-Ito semantics: sky-blue anchor for intact cell, green for the two preservation steps (fixation, osmium), vermillion for dehydration (the contraction/artifact-risk step), blue dominant for resin embed + ultramicrotomy, neutral/secondary for staining. Add a footnote: "Each step preserves some structure while risking artifact." Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables. Structural, not aesthetic.

### Figure 9.4 — Cryo-EM vitrification vs dried-TEM artifact
Build a two-panel before/after comparison. Left panel "Cryo-EM (vitrified)", right panel "Dried TEM". Render four aligned rows of paired statements: (1) amorphous vitreous ice preserves film vs water evaporates during drying; (2) particles stay dispersed/single vs particles clump into fused aggregates; (3) intact bilayer rings visible vs empty shells and lumps; (4) observed aggregation is real vs aggregation may be fabricated. Use Okabe-Ito semantics: green for the vitrified/preserved-truth side and the final "real" row; vermillion for the artifact rows (clumping, fabricated aggregation) on the dried-TEM side. Align rows on a shared baseline. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables. Structural, not aesthetic.
