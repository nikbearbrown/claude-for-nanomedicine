...
![Positioning map of six preclinical modalities by resolution versus field of view, from whole-body BLI to single-cell intravital two-photon](images/10-in-vivo-preclinical-imaging-fig-01.png)
*Figure 10.1 — Preclinical modality landscape*
...
![Longitudinal BLI workflow from engineering a luciferase cell line through orthotopic implant, weekly imaging, and photon-flux quantification, cross-checked by CT/MRI/PET](images/10-in-vivo-preclinical-imaging-fig-02.png)
*Figure 10.2 — Longitudinal orthotopic BLI workflow*
...
![Four-row concordance matrix showing BLI photon flux down against flat tumor volume, diffusion, and glucose metabolism, indicating signal dimming without cell kill](images/10-in-vivo-preclinical-imaging-fig-03.png)
*Figure 10.3 — When signals disagree: concordance matrix*
...
### Figure 10.1 — Preclinical modality landscape
Build a node-edge positioning map placing six preclinical modalities by resolution versus field of view: BLI (whole-body, enzyme light), in vivo fluorescence (fluorophore), microCT (X-ray anatomy), microMRI (NMR, info-rich), microPET (tracer function), intravital two-photon (single-cell). Position nodes on a two-axis layout — field of view on one axis, resolution on the other — so each modality occupies a distinct point on the tradeoff (no connecting edges required). Label each node with its signal source. Use Okabe-Ito semantics: sky-blue anchor for BLI, neutral/secondary for fluorescence, blue dominant for microMRI, transitional tone for microPET, green for intravital two-photon (single-cell). Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables. Structural, not aesthetic.

### Figure 10.2 — Longitudinal orthotopic BLI workflow
Build a left-to-right sequential flowchart of five numbered stages: (1) engineer luciferase cell line → (2) orthotopic implant → (3) luciferin + BLI imaging → (4) quantify photon flux over time → (5) cross-check (CT/MRI/PET). Render stages 1–4 joined by arrows; render stage 5 as a cross-confirm branch joined by a distinct labeled connector ("cross-confirm"). Use Okabe-Ito semantics: green for engineering the cell line, sky-blue anchor for orthotopic implant, blue dominant for BLI imaging, neutral/secondary for photon-flux quantification, transitional tone for the cross-check. Footnote: "Each transition is a proxy hand-off, not a direct readout of biology." Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables. Structural, not aesthetic.

### Figure 10.3 — When signals disagree: concordance matrix
Build a four-row comparison matrix with left column "Proxy" and right column "Status". Rows: BLI photon flux → Down; tumor volume (MRI) → Flat; diffusion → Flat; glucose metabolism (PET) → Flat. Align rows on a shared baseline so the concordance pattern reads at a glance — one down-signal against three flats. Use Okabe-Ito semantics: vermillion for the single "Down" BLI row (the deviating signal), neutral gray for the three "Flat" rows. Add a caption-level annotation: one down-signal against three flats means signal dimming without cell kill. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables. Structural, not aesthetic.
