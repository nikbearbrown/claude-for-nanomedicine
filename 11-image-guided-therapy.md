...
![Before/after spatial schematic showing a wide uncertainty margin engulfing organs versus a thin per-fraction-imaging margin that spares neighboring tissue](images/11-image-guided-therapy-fig-01.png)
*Figure 11.1 — Margin reduction lowers normal-tissue dose*
...
![Closed-loop cycle of localize in position, register to plan, reposition/adapt, and deliver beam, repeated every fraction to track daily anatomy](images/11-image-guided-therapy-fig-03.png)
*Figure 11.3 — Per-fraction localize-adapt-treat loop*
...
![Bar chart of relative dose at key depths comparing photon entrance and exit tail against the proton path, sharp Bragg peak at target, and near-zero dose beyond](images/11-image-guided-therapy-fig-02.png)
*Figure 11.2 — Photon vs proton dose-depth profiles*
...
### Figure 11.1 — Margin reduction lowers normal-tissue dose
Build a before/after two-panel spatial comparison. Left panel "Without image guidance", right panel "With per-fraction imaging". Render three aligned rows of paired statements: (1) wide uncertainty margin vs thin margin; (2) high-dose ring engulfs organs vs organs cleared of high-dose ring; (3) higher normal-tissue toxicity vs room to escalate tumor dose. Optionally show a concentric target-plus-margin schematic in each panel with margin width as the differentiating channel. Use Okabe-Ito semantics: vermillion for the wide-margin/high-toxicity left side, green for the spared-organ/escalation-room right side. Align rows on a shared baseline. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables. Structural, not aesthetic.

### Figure 11.2 — Photon vs proton dose-depth profiles
Build a vertical bar chart of relative dose at five key depth points: photon entrance (70), photon exit tail past target (30), proton entrance/path (25), proton Bragg peak at target (100), proton beyond peak (2). Zero baseline yes; keep the depth points in physical left-to-right order rather than sorting by value. Channel relative dose to bar height; highlight the proton Bragg peak as the dominant bar and mark "at target". Use Okabe-Ito semantics: neutral/secondary for photon entrance, vermillion for the photon exit tail (unwanted dose past target), blue dominant highlight for the Bragg peak, green for proton-beyond-peak (near-zero, the spared region). Add a reference annotation for the target depth. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables. Structural, not aesthetic.

### Figure 11.3 — Per-fraction localize-adapt-treat loop
Build a closed-loop cycle diagram of four stages arranged around a ring with directional arrows returning to the start: localize/image in position → register to plan → reposition/adapt → deliver beam → (back to localize). Label the loop as repeated every fraction to track daily anatomy. Use Okabe-Ito semantics: sky-blue anchor for localize/image, neutral/secondary for register to plan, transitional tone for reposition/adapt, green for deliver beam. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables. Structural, not aesthetic.
