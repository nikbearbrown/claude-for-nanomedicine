...
![Fan-in systems diagram of five evidence scales — anatomy, function, tissue, molecular, and systemic — converging on a single clinical decision](images/12-multimodal-integration-and-imaging-evidence-fig-01.png)
*Figure 12.1 — Multimodal evidence integration*
...
![Horizontal timeline of ctDNA level across diagnosis, treatment, surgery, surveillance MRD, resistance signal, and imaging-confirmed progression, with the re-rise leading imaging](images/12-multimodal-integration-and-imaging-evidence-fig-02.png)
*Figure 12.2 — ctDNA kinetics across the disease course*
...
![Two-panel contrast of bulk RNA-seq averaging a section into one profile versus spatial transcriptomics preserving tumor core, invasive front, immune infiltrate, and stroma zones](images/12-multimodal-integration-and-imaging-evidence-fig-03.png)
*Figure 12.3 — Bulk vs spatial profiling contrast*
...
![Vertical hierarchy ladder of diagnostic evidence rising from single-site retrospective radiomics through multi-site validation and prospective randomized trial to FDA-cleared narrow-task tool](images/12-multimodal-integration-and-imaging-evidence-fig-04.png)
*Figure 12.4 — Evidence-quality ladder for diagnostics*
...
### Figure 12.1 — Multimodal evidence integration
Build a node-edge systems diagram with five source nodes fanning in to one decision node. Sources: anatomy (CT/MRI), function (PET/diffusion), tissue (pathology/IHC), molecular (NGS), systemic (ctDNA/CTC); each with a directed arrow into a central "clinical decision" node. Arrange sources on one side converging on the decision. Label each source with its scale. Use Okabe-Ito semantics: sky-blue anchor for anatomy, neutral/secondary for function, transitional tone for tissue, blue dominant for molecular and for the decision node, green for systemic (ctDNA/CTC). Add a note that convergence is valid only when the sources' blind spots are independent. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables. Structural, not aesthetic.

### Figure 12.2 — ctDNA kinetics across the disease course
Build a horizontal timeline of six ordered markers along a disease-course axis with ctDNA level (log) as the vertical channel at each marker: diagnosis (high ctDNA), treatment (sharp fall, early response), surgery (below detection), surveillance MRD (re-rise detected), resistance signal (leads imaging), imaging confirms progression (lags ctDNA). Connect markers to trace the rise-fall-rise trajectory. Use Okabe-Ito semantics: green for the treatment early-response fall, sky-blue anchor for surgery/below-detection, vermillion for the surveillance re-rise and the resistance signal, neutral/secondary for imaging-confirmed progression. Annotate that the re-rise leads imaging. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables. Structural, not aesthetic.

### Figure 12.3 — Bulk vs spatial profiling contrast
Build a two-panel before/after comparison. Left panel "Bulk RNA-seq", right panel "Spatial transcriptomics". Render four aligned rows of paired statements: (1) section averaged to one profile vs position preserved; (2) architecture flattened vs tumor core zone resolved; (3) no regional detail vs invasive front and immune infiltrate seen; (4) single uniform readout vs stroma distinguished. Use Okabe-Ito semantics: vermillion for the averaging/architecture-lost left side, blue dominant and green and sky-blue anchor for the preserved-zone right side. Align rows on a shared baseline. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables. Structural, not aesthetic.

### Figure 12.4 — Evidence-quality ladder for diagnostics
Build a top-down hierarchy ladder of four evidence tiers from base to top: single-site retrospective radiomics (base) → multi-site validation → prospective randomized trial → FDA-cleared narrow-task tool (top). Render as a vertical hierarchy where height encodes evidence quality, distinguishing internal accuracy at the base from validated performance at the top. Use Okabe-Ito semantics: vermillion for the single-site retrospective base (weakest, internal-only), neutral/secondary for multi-site validation, sky-blue anchor for prospective randomized trial, green for the FDA-cleared top tier. Deliverable: single standalone HTML file, inline CSS, D3 v7 pinned CDN, Okabe-Ito colorblind-safe palette via CSS variables. Structural, not aesthetic.
